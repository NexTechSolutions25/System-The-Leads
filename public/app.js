...
function render() {
  ...
  $("#campaign-list").innerHTML = campaigns.length
    ? campaigns
        .map(
          (c) =>
            `<article class="card"><div class="card-top"><span class="chip">${esc(c.country)} · ${esc(c.scope)}</span><span class="badge">Até ${c.maxLeads} empresas</span></div><h2>${esc(c.name)}</h2><p>${esc(c.region || "Busca nacional")} ${c.cities.length ? "· " + esc(c.cities.join(", ")) : ""}</p><p>${esc(c.segment)} · ${esc(c.service)}</p><p>Corte ${c.minScore} · Máx. ${c.maxRequests} consultas · USD ${c.maxCost.toFixed(2)}</p><p>${c.schedule ? `${esc(c.schedule.frequency)} · ${esc(c.schedule.time)} · ${esc(c.schedule.timezone)} · ${c.scheduling?.active ? "Ativo" : "Inativo"}` : "Execução manual"}</p><div class="actions"><button class="primary" data-action="start" data-id="${c.id}">Iniciar captação</button>${c.schedule && !status.freeMode ? `<button data-action="${c.scheduling?.active ? "unschedule" : "schedule"}" data-id="${c.id}">${c.scheduling?.active ? "Pausar agenda" : "Ativar agenda"}</button>` : ""}</div></article>`,
        )
        .join("")
    : empty(
